...
int main()
{
	...
	for(lli i=1,q,a,b,x;i<=m;i++)
	{
		...
		if(q==1)
		{
			...
			update(a,-(a-1)*x,org);//a[1-n]+k
			update(a,x,added);//当x属于[1-n]时 sum=a[1-x]+x*k
			update(b,b*x,org);//当x>n时 sum=a[1-x]+n*k
			update(b,-x,added);//added[1]+=k,added[n]+=-k,org[n]+=n*k
							//因此sum=x*added[1-x]+org[1-x]
		}
		else if(q==2)
		{
			scanf("%lld%lld",&a,&b);
